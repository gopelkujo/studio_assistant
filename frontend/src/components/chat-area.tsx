  ...
  if (!activeSession) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center bg-zinc-950 text-zinc-400 h-full w-full">
        ...
        <h2 className="text-xl font-medium mb-2 text-zinc-300">Welcome to Studio Assistant</h2>
            ...
            <div key={msg.id} className={`flex gap-4 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              {msg.role === 'assistant' && (
                <Avatar className="w-8 h-8 rounded shrink-0 bg-indigo-600">
                  <AvatarFallback className="bg-indigo-600 text-white font-bold text-xs rounded">SA</AvatarFallback>
                </Avatar>
              )}
              <div className={`rounded-xl px-4 py-3 max-w-[85%] ${msg.role === 'user' ? 'bg-zinc-800 text-zinc-100 border border-zinc-700' : 'bg-zinc-900 text-zinc-200 border border-zinc-800 prose prose-invert prose-p:leading-relaxed'}`}>
              ...
              {msg.role === 'user' && (
                <Avatar className="w-8 h-8 shrink-0 bg-zinc-700">
                  <AvatarFallback className="bg-zinc-700 text-white font-medium text-xs">U</AvatarFallback>
                </Avatar>
              )}
            </div>
          ))}
        ...
        <div className="max-w-3xl mx-auto">
          {activeSession.messages.length === 0 && (
            <div className="flex gap-2 mb-3">
              <Button type="button" onClick={() => insertCommand('/narrative')} className="h-7 text-xs bg-zinc-900 border border-zinc-800 text-zinc-300 hover:bg-zinc-800 hover:text-zinc-100">
                <Wand2 className="w-3 h-3 mr-1" /> /narrative
              </Button>
              <Button type="button" onClick={() => insertCommand('/asset-brief')} className="h-7 text-xs bg-zinc-900 border border-zinc-800 text-zinc-300 hover:bg-zinc-800 hover:text-zinc-100">
                <Box className="w-3 h-3 mr-1" /> /asset-brief
              </Button>
            </div>
          )}
          
          <form onSubmit={handleSubmit} className="relative flex items-end gap-2 bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden focus-within:ring-1 focus-within:ring-zinc-600 focus-within:border-zinc-600 transition-all p-2 shadow-inner">
            <Textarea 
              value={input}
          ...
          <div className="text-center mt-2">
             <span className="text-[10px] text-zinc-600">AI outputs may not be accurate. Verify narrative lore before production.</span>
          </div>
        </div>
      </div>
    </div>
